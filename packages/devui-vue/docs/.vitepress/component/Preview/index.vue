...
import { ref, onUnmounted, watchEffect, watch, version } from 'vue';
import type { WatchStopHandle } from 'vue';
import srcdoc from './srcdoc.html?raw';
import { PreviewProxy } from './PreviewProxy';
// import { compileModulesForPreview } from '../../utils/moduleCompiler';
import { getDefaultImportConfig } from '../../utils/config';
...
let codebox: HTMLIFrameElement
let proxy: PreviewProxy

// console.log(srcdoc, 'srcdoc');
let stopUpdateWatcher: WatchStopHandle | undefined

// // create codebox on mount
// onMounted(createSandbox)

// reset codebox when import map changes
watch(
  () => props.value,
  (code) => {
    try {
...
// reset codebox when version changes
...
function createSandbox() {
  if (codebox) {
    ...
    proxy.destroy()
    stopUpdateWatcher && stopUpdateWatcher()
    container.value.removeChild(codebox)
  }

  codebox = document.createElement('iframe')
  ...
  const importMap = getDefaultImportConfig({ version });

  const codeboxSrc = srcdoc.replace(
    /<!--IMPORT_MAP-->/,
    JSON.stringify(importMap)
  )
  codebox.srcdoc = codeboxSrc
  container.value.appendChild(codebox)

  proxy = new PreviewProxy(codebox, {
...
<template>
  <div class="iframe-container" ref="container">hello</div>
